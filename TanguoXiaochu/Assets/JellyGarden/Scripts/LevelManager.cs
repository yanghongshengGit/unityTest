...
public class LevelManager : MonoBehaviour
{
		...
		set {
			...
			if (value == GameState.PrepareGame) {
				...
				if (InitScript.Instance.ShowChartboostAdsEveryLevel > 0) {
					if (passLevelCounter % InitScript.Instance.ShowChartboostAdsEveryLevel == 0 && InitScript.Instance.enableAds)
						InitScript.Instance.ShowAds (true);
				}
				if (InitScript.Instance.ShowAdmobAdsEveryLevel > 0) {
					if (passLevelCounter % InitScript.Instance.ShowAdmobAdsEveryLevel == 0 && InitScript.Instance.enableAds)
	...
	IEnumerator PreWinAnimationsCor ()
	{
		...
		foreach (Item item in items) {
            if (limitType == LIMIT.MOVES) Limit--;
	...
	void Update ()
	{
        ...
        {
            if (LevelManager.THIS.gameStatus == GameState.Playing) GameObject.Find("CanvasGlobal").transform.Find("MenuPause").gameObject.SetActive(true);
            else if (LevelManager.THIS.gameStatus == GameState.Map) Application.Quit();
        }
